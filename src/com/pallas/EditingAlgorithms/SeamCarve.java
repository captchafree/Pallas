...
public class SeamCarve implements EditAlgorithm {
    ...
    public BufferedImage performEdit(BufferedImage image) {
        ...
        System.out.println("Getting gradient X matrix");
        int[][] gradientX = getGradientMatrixX(matrix);
        System.out.println("Getting gradient Y matrix");
        int[][] gradientY = getGradientMatrixY(matrix);

        System.out.println("Computing composite");
        ...
        System.out.println("Creating composite Image");
        for(int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
